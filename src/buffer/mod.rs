...
impl ReceiveBuffer {
    ...
    pub fn add_data_packet(&self, packet: Packet) -> Result<bool, String> {
        ...
        let batch_buffer = batches
            .entry(packet.header.batch_number)
        ...
        batch_buffer
            .packets
            .insert(packet.header.packet_in_batch, packet);

        // Проверяем, получили ли мы все пакеты партии
        let is_complete = batch_buffer.packets.len() == batch_buffer.expected_count as usize
            && batch_buffer
                .packets
                .contains_key(&(batch_buffer.expected_count - 1))
            && batch_buffer.packets[&(batch_buffer.expected_count - 1)]
                .header
                .is_last_in_batch();
    ...
    pub fn get_completed_batch(
        &self,
        batch_number: u32,
    ) -> Option<(Vec<Packet>, Vec<blake3::Hash>)> {
