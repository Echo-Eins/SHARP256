...
impl ReceiverApp {
    pub fn new() -> Self {
        ...
        let output_dir = dirs::download_dir().unwrap_or_else(|| PathBuf::from("./received"));

        let state = Arc::new(RwLock::new(ReceiverState::Listening(
        ...
        if let Some(incoming) = &*self.show_incoming_dialog.read() {
            egui::Window::new("Incoming Transfer")
                ...
                .show(ctx, |ui| {
                    ...
                    ui.label(format!(
                        "Size: {:.2} MB",
                        incoming.file_size as f64 / 1024.0 / 1024.0
                    ));
                    ...
                    if transfers.is_empty() {
                        ...
                        for transfer in transfers.iter().rev() {
                            ui.group(|ui| {
                                ...
                                ui.label(format!(
                                    "Size: {:.2} MB",
                                    transfer.size as f64 / 1024.0 / 1024.0
                                ));
