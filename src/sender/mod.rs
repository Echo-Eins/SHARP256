/// Возобновление передачи
    async fn resume_transfer(&self, saved_state: TransferState) -> Result<()> {
        ...
        *self.transfer_state.write() = saved_state.clone();
        ...
        // Отправляем запрос на возобновление
        let resume_ack = crate::protocol::ack::ResumeAck {
            file_name: saved_state.file_name.clone(),
            file_size: saved_state.file_size,
            last_batch_number: saved_state.last_batch_number,
            last_packet_in_batch: saved_state.last_packet_in_batch,
            bytes_transferred: saved_state.bytes_transferred,
            partial_file_hash: saved_state.partial_file_hash.clone()
                .unwrap_or_else(|| "unknown".to_string()),
            resume_token: saved_state.resume_token.clone(),
        };
        
        let ack_bytes = bincode::serialize(&resume_ack)?;
            ...
            match timeout(Duration::from_secs(10), self.socket.recv_from(&mut buffer)).await {
                Ok(Ok((size, addr))) if addr == self.peer_addr => {
                    ...
                    if let Ok(response_packet) = Packet::from_bytes(&mut buf) {
                        if response_packet.header.packet_type == PacketType::Resume {
                            let response: crate::protocol::ack::ResumeAckResponse = 
                                bincode::deserialize(&response_packet.payload)?;
                            
                            if response.can_resume {
                                tracing::info!("Resume accepted from batch {} packet {}", 
                                    response.resume_from_batch, response.resume_from_packet);
                                
                                // Запускаем передачу с указанной позиции
                                self.transfer_file_from_position(
                                    response.resume_from_batch,
                                    response.resume_from_packet,
                                ).await?;
                                return Ok(());
                            } else {
                                tracing::warn!("Resume rejected: {:?}", response.reason);
                                // Начинаем заново
                                return self.new_transfer().await;
                            }
                        }
                    }
                }
                _ => {
                    tracing::warn!("Resume ACK attempt {} failed", attempt + 1);
                }
            }
        }
        
        // Если не удалось возобновить, начинаем заново
        tracing::warn!("Failed to resume, starting new transfer");
        self.new_transfer().await
    }
    
    /// Передача файла с определенной позиции (для возобновления)
    async fn transfer_file_from_position(
        &self,
        start_batch: u32,
        start_packet_in_batch: u16,
    ) -> Result<()> {
        ...
        self.transfer_state.write().last_batch_number = start_batch;
        self.transfer_state.write().last_packet_in_batch = start_packet_in_batch;
        ...
        // Основной цикл чтения с позиции возобновления
        self.file_reader_task_from_position(tx_packets, hash_complete, start_batch, start_packet_in_batch).await?;
    ...
    async fn file_reader_task_from_position(
        &self,
        ...
        let payload_size = if *self.use_gso.read() {
            MAX_PAYLOAD_SIZE_GSO
        } else {
            MAX_PAYLOAD_SIZE_MTU
        };
        
        // Вычисляем начальную позицию в файле
        let packets_before = start_batch as u64 * batch_size as u64 + start_packet_in_batch as u64;
        let mut offset = packets_before * payload_size as u64;
        
        let mut global_sequence = packets_before as u32;
        let mut current_batch = start_batch;
        let mut packet_in_batch = start_packet_in_batch;
        
        tracing::info!("Resuming from offset {} (batch {} packet {})", offset, current_batch, packet_in_batch);
        
        while offset < file_size {
            // Читаем данные для пакета
            let read_size = payload_size.min((file_size - offset) as usize);
            let data = self.file_manager.read_at(offset, read_size)?;
            
            let mut header = PacketHeader::new(PacketType::Data);
            header.batch_number = current_batch;
            header.packet_in_batch = packet_in_batch;
            header.total_packets = batch_size;
            header.payload_length = data.len() as u32;
            header.sequence = global_sequence;
            
            // Проверяем, последний ли это пакет в партии
            if packet_in_batch == batch_size - 1 {
                header.set_last_in_batch();
            }
            
            let packet = Packet::new(header, data.clone());
            
            // Сохраняем в буфер для возможного перезапроса
            self.packet_buffer.add_packet(current_batch, packet_in_batch, packet.clone());
            
            // Отправляем пакет
            tx_packets.send(packet).await?;
            
            // Обновляем состояние
            self.transfer_state.write().update_progress(
                current_batch,
                packet_in_batch,
                data.len() as u64,
                Some(&data),
            ...
            if packet_in_batch >= batch_size {
                packet_in_batch = 0;use anyhow::{Context, Result};
use blake3::Hasher;
use bytes::BytesMut;
