...
impl TransferState {
    pub fn new(file_name: String, file_size: u64, is_sender: bool, peer_address: String) -> Self {
    ...
    pub fn save(&self, state_dir: &Path) -> Result<PathBuf> {
        ...
        let json =
            serde_json::to_string_pretty(self).context("Failed to serialize transfer state")?;
    ...
    pub fn load(state_file: &Path) -> Result<Self> {
        ...
        let state = serde_json::from_str(&json).context("Failed to deserialize transfer state")?;
    ...
    pub fn find_latest(
        ...
        let pattern = format!("{}_{}", file_stem, if is_sender { "send" } else { "recv" });
    ...
    pub fn find_resume_position(
        &self,
        file_manager: &crate::file::FileManager,
    ) -> Result<Option<u64>> {
        if self.last_data_fragment.is_empty() {
            return Ok(None);
        }

        let fragment = general_purpose::STANDARD
            .decode(&self.last_data_fragment)
